...
public class AudioMain {
    ...
    private native static int pcm16leToWav(String pcmPath, String wavPath, int byteType,int channel); // 1:小端字节 0：大端字节
    ...
    public static void pcmToWav(final String pcmPath, final String wavPath, final HandleCallback handleCallback) {
        if (handleCallback != null) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    handleCallback.onBegin();
                    int result = pcm16leToWav(pcmPath, wavPath, 0,2);
                    handleCallback.onEnd(result);
                }
            }).start();
        }
